...
5. **resolve-one SNS is a fallback after API Gateway, not a parallel source.** The design proposed a multi-source chooser. The implementation fires SNS resolution only when API Gateway resolution fails AND SNS IaC signals are present. Manual-review SNS results are skipped (tries next topic).
...
This document proposes how `postman-aws-spec-discovery-action` should add **AWS SNS** as a first-class provider for **event-driven contract resolution**, with a focus on the Fox requirement to detect **contract drift** across publisher/consumer boundaries rather than only discovering HTTP endpoints.
...
Fox's architecture uses large numbers of SNS topics to propagate events between services. Their primary failure mode is not simple uptime loss. It is:
...
The main drift signal Fox wants is:
...
That gives Fox the missing capability they asked for:
...
This is the minimum viable path that aligns the AWS action with Fox's core requirement: **event-driven contract drift detection**, not just resource discovery.
