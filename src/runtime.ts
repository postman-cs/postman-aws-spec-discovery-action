...
export function readActionInputs(inputReader: InputReaderLike): ResolvedInputs {
  ...
  return resolveInputs({
    ...
    INPUT_REPO_ROOT:
      normalizeInputValue(inputReader.getInput('repo-root')) ?? actionContract.inputs['repo-root'].default,
    INPUT_REPO_URL: normalizeInputValue(inputReader.getInput('repo-url')),
    INPUT_REPO_SLUG: normalizeInputValue(inputReader.getInput('repo-slug')),
    INPUT_GIT_PROVIDER: normalizeInputValue(inputReader.getInput('git-provider')),
    INPUT_REF: normalizeInputValue(inputReader.getInput('ref')),
    INPUT_SHA: normalizeInputValue(inputReader.getInput('sha')),
    INPUT_EXPECTED_SERVICE_NAME: normalizeInputValue(inputReader.getInput('expected-service-name')),
    INPUT_EXPECTED_GATEWAY_IDS_JSON:
      normalizeInputValue(inputReader.getInput('expected-gateway-ids-json')) ??
      actionContract.inputs['expected-gateway-ids-json'].default,
    INPUT_STAGE: normalizeInputValue(inputReader.getInput('stage')),
    INPUT_API_FILTER: normalizeInputValue(inputReader.getInput('api-filter')),
    INPUT_SERVICE_MAPPING_JSON:
      normalizeInputValue(inputReader.getInput('service-mapping-json')) ??
      actionContract.inputs['service-mapping-json'].default,
    INPUT_OUTPUT_DIR:
      normalizeInputValue(inputReader.getInput('output-dir')) ?? actionContract.inputs['output-dir'].default,
    INPUT_INCLUDE_V2:
      normalizeInputValue(inputReader.getInput('include-v2')) ?? actionContract.inputs['include-v2'].default,
    ...process.env
  });
}

function resolveLegacyServiceName(
  gatewayId: string,
  gatewayName: string,
...
async function resolveStageSelection(
  ...
  const stages =
    candidate.gatewayType === 'REST'
      ? await aws.listRestStages(candidate.id)
      : await aws.listHttpStages(candidate.id);
