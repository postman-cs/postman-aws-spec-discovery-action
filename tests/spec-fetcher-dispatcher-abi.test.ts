...
// opened, so every remote spec URL fetch failed at runtime while the mocked unit tests
// stayed green. Reproduced on Node 22, 24, and 25.
//
...
describe('pinned dispatcher and fetch come from one undici instance', () => {
  ...
  it('defaults fetchSpecFromUrl to an implementation that accepts its own dispatcher', async () => {
    const { enableRemoteFetch, fetchSpecFromUrl } = await import('../src/lib/fetch/spec-fetcher.js');
    const policy = enableRemoteFetch([{ hostname: HOST, pathPrefix: '/v1/' }]);

    const chain = await failureChain(() =>
      fetchSpecFromUrl(URL_, {
        policy,
        timeoutMs: 5000,
        lookup: async () => [{ address: PUBLIC_ADDRESS, family: 4 as const }]
      })
    );

    // Must fail on transport, never on handler validation.
    expect(chain).not.toContain(ABI_REJECTION);
    expect(chain).toContain('fetch failed');
  }, 30000);
});
