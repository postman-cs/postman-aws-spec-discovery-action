...
`validate-support-ledger.mjs` fails if advertised README/providers support lacks ledger/test/evidence mapping. `validate-resolution-closure.mjs` covers Fox tag fixtures, JSON Schema/Avro selection fixtures, Smithy project closure, GraphQL grouping, monorepo/Backstage ambiguity, static IaC fixtures, adversarial remote/local boundaries, provenance schema examples, and deterministic ordering/limits via fixture checks plus the named vitest suites.
