...
| `tag-github-org-repo-split` | Exact the customer GithubOrg+GithubRepo tag correlation | full | src/lib/resolve/narrowing-pipeline.ts<br>src/lib/aws/tagging-client.ts | tests/narrowing.test.ts<br>tests/tag-correlation.test.ts | validate-resolution-closure.mjs#github-org-repo-split | required/passed | full |
