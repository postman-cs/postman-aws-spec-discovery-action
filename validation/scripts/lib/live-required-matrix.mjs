...
export const REQUIRED_LIVE_CASE_DEFS = [
  {
    ...
    ledgerIds: ['tag-postman-repo', 'tag-github-org-repo-split'],
    description: 'Checked-out GithubOrg/GithubRepo-tagged service repo resolves intended gateway with only ambient AWS credentials/region and CI repo identity (no explicit gateway/service input).'
  },
  {
    id: 'github-org-repo-multi-environment-ambiguity',
    ledgerIds: ['tag-multi-environment-ambiguity'],
    description: 'Exact multi-environment GithubOrg/GithubRepo tag duplicates remain ranked manual-review unless explicit evidence selects one.'
  },
  {
